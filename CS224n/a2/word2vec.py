...
def naiveSoftmaxLossAndGradient(
    ...
    gradCenterVec = -outsideVectors[outsideWordIdx] + np.sum(outsideVectors*tmp[:, np.newaxis], axis=0)
    gradOutsideVecs = (tmp - (np.arange(len(tmp))==outsideWordIdx))[:, np.newaxis]*centerWordVec[np.newaxis, :]
    ### END YOUR CODE
...
def negSamplingLossAndGradient(
    ...
    mask = np.array([1]+[-1]*K)
    sigmoid_lst = np.array([sigmoid(mask[i]*np.dot(centerWordVec, outsideVectors[indices[i]])) for i in range(K+1)])
    loss = -np.sum(np.log(sigmoid_lst))
    gradCenterVec = -np.sum([mask[i]*(1-sigmoid_lst[i])*outsideVectors[indices[i]] for i in range(K+1)], axis=0)
    gradOutsideVecs = np.zeros_like(outsideVectors)
    for i in range(len(indices)):
        gradOutsideVecs[indices[i]] += mask[i]*(sigmoid_lst[i]-1)*centerWordVec
    ### END YOUR CODE
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for outside_word in outsideWords:
        loss_1, grad_1, grad_2 = word2vecLossAndGradient(centerWordVectors[currentCenterWordIdx], word2Ind[outside_word], outsideVectors, dataset)
        loss += loss_1
        gradCenterVecs[currentCenterWordIdx] += grad_1
        gradOutsideVectors += grad_2
    ### END YOUR CODE
